...
const Update = () => {
          const {setUpdateProductForm,getOneProduct,getAllProducts,setProductModel,setRespText} = useGlobalContext();
          const [productData,setProductData] = useState(getOneProduct[0]);

  const onHandleSubmit =async(e)=>{
    e.preventDefault();

    updateProduct(productData.id,productData)
    .then(resp=>setRespText(resp.data.message))
    setUpdateProductForm(false)
  ...
  return (
            ...
            <div className='pb-5'>
              <h1 className='uppercase text-lg font-semibold text-center font-serif tracking-wide text-gray-700'>Product Update</h1>

            <button className='fixed top-2 right-2 bg-red-400 text-white hover:bg-red-700 p-1 drop-shadow-2xl text-xl rounded-full' onClick={()=>setUpdateProductForm(false)}>
                <AiOutlineClose />
            </button>
            </div>
            ...
            <button className='w-full bg-blue-500 rounded-lg my-2 text-white py-1 text-lg hover:bg-blue-800 hover:drop-shadow-2xl'>Update Product</button>
        </form>
            </div>
            </div>
  )
